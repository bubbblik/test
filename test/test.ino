...
RTC_DS3231 rtc; //создаем объект rtc для модуля времени
BME280_I2C bme; //создаем объект bme для датчика температуры
MQ135 gasSensor = MQ135(A1); //создаем объект gasSensor для датчика СО2
...
LiquidCrystal_I2C lcd(0x27,20,4); //создаем объект lcd для дисплея

char daysOfTheWeek[7][12] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"}; //названия дней недели
int aFlag=0; // флаг для предупреждении о СО2
...
void firstPage(){
  ...
  if (millis()-counter_set>=5000 || dop == 1){; //millis() - функция, которая считает время от запуска программы. 
                                                ...
                                                //т.к. от запуска программы просто не пройдет столько времени). для отрисовки пришлось
          ...
          lcd.setCursor(0,0); //установка курсора на 1 столбец, 1 строку (всего 20 столбцов и 4 строки)
          lcd.write(1); //знак температуры
          lcd.print(" Tem: "); //вывод текста " Tem: "
          lcd.print(bme.getTemperature_C());//принимаем и виводим  значение от датчика температуры
          lcd.print(" C");  //вывод текста " C"
